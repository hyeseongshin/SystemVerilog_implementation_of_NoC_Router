...
module output_buffer_tb;
    ...
    task automatic drive_st2obuf(int iters);
        int random_data;
        ...
        for (int i = 0; i < iters; i++) begin
            ...
            obuf_if.driver_cb.in_valid <= 1'b1;
            obuf_if.driver_cb.in_data <= flit_t'(random_data);

            do begin
                @(obuf_if.driver_cb);
            end while(obuf_if.num_free == 0);
        end
        obuf_if.driver_cb.in_valid <= 1'b0;
    endtask


    task automatic drive_network2obuf(bit has_back_pressure);
        @(obuf_if.driver_cb);
        forever begin
            if (has_back_pressure) begin
                obuf_if.driver_cb.out_ready <= 1'b0;
                repeat($urandom_range(100, 10)) @(obuf_if.driver_cb);
                obuf_if.driver_cb.out_ready <= 1'b1;
            end
            else
                obuf_if.driver_cb.out_ready <= 1'b1;
            ...
            do begin
                @(obuf_if.driver_cb);
            end while(obuf_if.driver_cb.out_valid == 1'b0);
        end
    endtask


    task automatic monitoring_obuf();
        flit_t expected_data;

        forever begin
            @(obuf_if.monitor_cb);
            if (obuf_if.monitor_cb.out_valid && obuf_if.monitor_cb.out_ready) begin
                ...
                if (obuf_if.monitor_cb.out_data != expected_data)
